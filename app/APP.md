...
- **The deploy grant is live and correctly scoped.** `sudo -n -l` as `rami` shows exactly one
  `NOPASSWD` entry — `(root) NOPASSWD: /opt/logbook/logbook-apply ""` — and general `sudo -n true`
  still **exits 1**, so passwordless root did not widen. `/opt/logbook/logbook-apply` is `root:root
  0755`, and `/etc/sudoers.d/` is `0700` (an `ls` from `rami` is correctly denied).
- **⛔ TASK 22 IS FINALLY CLOSED — the Apache half is applied.** All four `/logbook` paths — the
  index, `sw.js`, `manifest.webmanifest` and the hashed bundle — now answer
  `no-cache, no-store, must-revalidate, max-age=0` with **no `ETag` at all**. The hashed asset had
  been served `public, max-age=31536000, immutable` for three weeks after the ruling that abolished
...
1. ✅ **DONE 2026-09-06 — the Apache half of Task 22 is applied and verified** (see above). The text
   below is kept for the history of *why it hid for three weeks*; it is no longer a task.

   ~~⛔ **THE APACHE HALF OF TASK 22 IS STILL NOT APPLIED.**~~ This is the one real piece of outstanding
